...
- [Data and Model Checkpoints](#data-and-model-checkpoints)
...
**Q:** _"ModuleNotFoundError: No module named 'sam2'" or import errors for other custom packages_  
**A:**  
- Ensure the `sam2` library is installed and discoverable (if it's your own code, make sure the repo is cloned or symlinked into your environment, or add its path to `PYTHONPATH`).
- If using external project code, install the package in "editable" mode with `pip install -e .` from the `sam2` directory.
...
**Q:** _IoU is suspiciously low (near zero) for all products_  
**A:**  
- Check that bounding box extraction code works as expected on both predicted and ground-truth masks (visualize random samples!).
- Make sure your prompt and test images/masks truly correspond to the same product (no shuffling or accidental mis-indexing).
- Carefully check preprocessing steps—wrong dimension order or type (`uint8` vs `bool` vs `float32`) may degrade mask outputs.
---

## Data and Model Checkpoints

Due to file size constraints, the following are NOT included in the repository:

- The CMU10_3D dataset (~2GB). Please download it from the [official site](<link>) and place in `CMU10_3D/data_2D/`.
- The SAM2 model checkpoint (`sam2_hiera_tiny.pt`) and config. Please download from [Meta AI link](<link>) and [config link](<link>), and place in the `checkpoints/` folder.
- Large results (plots/CSVs) can be generated by running the notebook as described.

For reproducibility, follow instructions in the notebook and README to set up these dependencies locally.
